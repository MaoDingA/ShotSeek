...
| `step-3.7-flash` 原生视频输入 | 1 秒、2 秒 MP4 均 HTTP 200 | 通过 |
...
Provider 已固定显式非流式请求、`reasoning_effort=low` 和 `max_tokens=2048`。此前 5～75 秒请求的 `engine_exception` 说明当前服务仍存在时长或资源边界，后续必须逐级扩大时长并记录上限，但原生视频能力本身已经确认可用。
